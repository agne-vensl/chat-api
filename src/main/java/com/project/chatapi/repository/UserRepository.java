...
public interface UserRepository extends JpaRepository<User, Long> {
  @Query(value = """
    SELECT * FROM users u WHERE public_id = :publicId
    """, nativeQuery = true)
  public Optional<User> findByPublicId(@Param("publicId") UUID publicId);

  @Query(value = """
    SELECT * FROM users u WHERE public_id = :publicId AND deleted = false
    """, nativeQuery = true)
  public Optional<User> findActiveUserByPublicId(@Param("publicId") UUID publicId);

  @Query(value = """
    SELECT * FROM users u 
    WHERE LOWER(u.username) = LOWER(:username) AND deleted = false
    """, nativeQuery = true)
  public Optional<User> findActiveUserByUsername(@Param("username") String username);

  @Modifying
  @Query(value = """
    INSERT INTO users (public_id, username, password, role, deleted) 
    VALUES (:publicId, :username, :password, :role, :deleted)
    """, nativeQuery = true)
  ...
  @Query(value = """
    UPDATE users 
    SET deleted = true, username = 'anonymous user' 
